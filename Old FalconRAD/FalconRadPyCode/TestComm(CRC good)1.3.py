...
def send_packet():
    ...
    reqpacket_1 = b'\xff\x12\x12\x12\x12\x13\xff\xff\xff'            #create initial packet
    crc = crc8.crc8()
    crc.update(b'\xff\x12\x12\x12\x12\x13\xff\xff\xff')
    calcdcrc = crc.digest()
    packet_final = reqpacket_1+calcdcrc
    #globals()['port'].write(reqpacket_1)

    #icsumb = icsum.to_bytes(4,'big')
    #final = 0xFF.to_bytes(1,'big')
    #print(icsumb)                         #convert checksum to bytes
    #packet_final = reqpacket_1+icsumb+final    #make one list of bytes with both the packet at checksum bytes
...
def receive_packet():
    ...
    if globals()['port'].in_waiting:      #if there are bytes waiting 
        reply = globals()['port'].read(50) #read the reply from arduino
